...
<div class="snbWrap" id="snb">
	...
	<c:forEach var="menu" items="${user.selectedTopMenu.childMenuMngts}">
		...
		<li <c:if test="${menu.menuMngtSeq eq user.selectedCurrMenu.menuMngtSeq || menu.menuMngtSeq eq user.selectedCurrMenu.upMenuMngtSeq}">class="on"</c:if>>
			...
			<c:if test="${!empty menu.childMenuMngts}">
				...
				<li <c:if test="${menu3.menuMngtSeq eq user.selectedCurrMenu.menuMngtSeq}">class="on"</c:if>>
